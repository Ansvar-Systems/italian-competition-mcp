# ─────────────────────────────────────────────────────────────────────────────
# AGCM Italian Competition MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t italian-competition-mcp .
# Run:    docker run --rm -p 3000:3000 italian-competition-mcp
#
# The image bakes /app/data/agcm.db at build time. Override AGCM_DB_PATH
...
ENV AGCM_DB_PATH=/app/data/agcm.db
...
# Bake the pre-built database into the image so /app/data/agcm.db resolves
...
# We then COPY it into the image at /app/data/agcm.db (AGCM_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/agcm.db
